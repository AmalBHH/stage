...
export default function presence() {
  ...
  const getData = async () => {
    try {
      const docRef = collection(db, "pres");
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.buttonRow}>
        <TouchableOpacity style={[styles.topButton, styles.inactiveButton]} onPress={() => navigation.navigate('newpres')}>
          <Text style={styles.buttonText}>Créer</Text>
        </TouchableOpacity>
      </View>
      <View style={styles.secondaryButtons}>

        <Feather name="list" size={24} color="gray"  onPress={() => navigation.navigate('presence')}/>
        <Feather name="grid" size={24} color="black" onPress={() => navigation.navigate('preskaban')}/>

      </View>
      <View style={styles.tableContainer}>
        <View style={styles.tableHeader}>
          <Text style={styles.headerText}>Nom</Text>
          <Text style={styles.headerText}>Arrivé</Text>
          <Text style={styles.headerText}>Sortie</Text>
          <Text style={styles.headerText}>Heures de Travail</Text>

        </View>
        {userInfo && userInfo.map((user: any, index: number) => (
          <View key={index} style={styles.tableRow}>
            <Text style={styles.rowText}>{user.empname}</Text>
            <Text style={styles.rowText}>{user.arrive}</Text>
            <Text style={styles.rowText}>{user.sortie}</Text>
            <Text style={styles.rowText}>{user.hdetravail}</Text>
